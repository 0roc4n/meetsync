    ...
    <body class="bg-#e0e0e2">
        ...
        <div class="pb-16 animate-fade_in">
            ...
            @if(session('error_message'))
                <div class="px-10 mt-4">
                    <div id="error-alert" class="bg-red-500 text-white p-4 rounded">
            ...
            <div class="px-10 mt-7 font-inter">
                <div class="flex space-x-7">
                    ...
                    class="relative group 
                        ...
                        {{ Request::routeIs('meetings.all') ? 'font-bold text-black' : 'font-medium' }} 
                        ...
                        <span class="block absolute bottom-[-10px] left-0 w-full h-1 bg-black 
                                    {{ Request::routeIs('meetings.all') ? 'scale-x-100' : 'scale-x-0' }} 
                    ...
                    <!-- PENDING -->
                    <a href="{{ route('meetings.pending') }}" 
                    class="relative group 
                        text-[#171717]-73 hover:text-black 
                        {{ Request::routeIs('meetings.pending') ? 'font-bold text-black' : 'font-medium' }} 
                        hover:font-bold animate-none">
                        <span class="relative z-10">Pending</span>
                        <span class="block absolute bottom-[-10px] left-0 w-full h-1 bg-black 
                                    {{ Request::routeIs('meetings.pending') ? 'scale-x-100' : 'scale-x-0' }} 
                                    group-hover:scale-x-100 
                                    transition-transform duration-300 ease-in-out">
                        </span>
                    </a>
                    ...
                    <!-- DONE -->
                    <a href="{{ route('meetings.done') }}" 
                    class="relative group 
                            text-[#171717]-73 hover:text-black 
                            {{ Request::routeIs('meetings.done') ? 'font-bold text-black' : 'font-medium' }} 
                            hover:font-bold animate-none">
                        <span class="relative z-10">Done</span>
                        <span class="block absolute bottom-[-10px] left-0 w-full h-1 bg-black 
                                    {{ Request::routeIs('meetings.done') ? 'scale-x-100' : 'scale-x-0' }} 
                                    group-hover:scale-x-100 
                                    transition-transform duration-300 ease-in-out">
                        </span>
                    </a>
                            ...
                            <div class="card-content p-7">
                                ...
                                <div class="flex justify-end mb-3">
                                    ...
                                    <div class="dropdown-menu hidden absolute left-13 mt-8 w-40 bg-[#ececec] border border-gray-300 rounded-lg shadow-lg">
                                        <ul class="py-1">
                                            <li class="my-1">
                                                <a href="{{ url('/edit_notes/' . $meeting->id) }}" 
                                                class="block px-2 py-2 text-sm text-gray-700 hover:bg-[#e6e6e6] hover:rounded-lg hover:shadow-md hover:font-medium ml-2 mr-2">
                                                    <!-----Edit Notes---> Open
                                                </a>
                                            </li>
                                            <form action="{{ url('/delete_meeting/' . $meeting->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this meeting?');" class="my-1 ml-2 mr-2">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        class="block w-full text-left px-2 py-2 text-sm text-gray-700 hover:bg-[#e6e6e6] hover:rounded-lg hover:shadow-md hover:font-medium">
                                                    Delete 
                                                </button>
                                            </form>
                                        </ul>

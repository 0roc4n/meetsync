    ...
    <div class="h-full p-5 flex flex-col">
        ...
        <nav class="font-inter">
            <ul class="flex flex-col">
                <li>
                    <a href="/homem" class="flex items-center px-3 py-3 hover:bg-#333333-90 hover:rounded-md hover:cursor-pointer group">
                    ...
                    <a href="/meetingsm" class="flex items-center px-3 py-3 hover:bg-#333333-90 hover:rounded-md hover:cursor-pointer group">
                    ...
                    <a href="/account_settingsm" class="flex items-center px-3 py-3 hover:bg-#333333-90 hover:rounded-md hover:cursor-pointer group">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" class="w-6 h-5 mr-5">
        ...
        <div class="bg-[#33333340] p-5 text-white text-center mt-auto rounded-md">
            <div class="flex">
                <!-- Profile Circle (Prevent resizing) -->
                @isset($info)
                    <div class="w-11 h-11 rounded-full bg-gray-500 flex justify-center mr-3 flex-shrink-0">
                        <img src="{{ asset($info->profile_picture) }}" alt="Profile Picture" class="w-full h-full object-cover rounded-full">
                    </div>
                @endisset
                <!-- Text Column with Scrollable Text -->
                <div class="flex flex-col items-start font-inter max-w-[calc(100%-3rem)] overflow-x-auto">
                    @isset($info)
                        <span class="text-base font-semibold inline-block whitespace-nowrap">{{ $info->first_name }} {{ $info->last_name }}</span>
                        <span class="text-xs font-light inline-block whitespace-nowrap">{{ $info->email }}</span>
                    @endisset
                </div>
            </div>
        </div>
